...
**🐛 บั๊กที่ผู้ใช้เจอบนเว็บจริงแล้วแก้ทันที (2026-09-16):** พิมพ์ข้อความใน dialog แล้ว**โฟกัสเด้งกลับไปช่อง dropdown ทุกตัวอักษร**
สาเหตุ: effect ที่สั่งโฟกัสตอนเปิด dialog (เพิ่งเพิ่มใน Phase E) มี deps เป็น `[onClose]` ซึ่ง component แม่สร้างฟังก์ชันใหม่ทุก render
→ ทุกตัวอักษรที่พิมพ์ทำให้ effect ทำงานซ้ำและโฟกัสถูกดึงกลับไปช่องแรก
แก้โดยแยกเป็น 2 effect (โฟกัส deps `[]` · ตัวดักปุ่มคีย์บอร์ด deps `[onClose]`) — ทดสอบแล้ว: พิมพ์ 15 ตัวอักษรอยู่ครบ โฟกัสไม่ขยับ · Esc ยังปิดได้ · คืนโฟกัสให้ปุ่มที่เปิดถูกตัว
**บทเรียน:** effect ที่ "ทำครั้งเดียวตอนเปิด" ห้ามผูก deps กับฟังก์ชันที่แม่สร้างใหม่ทุก render
