...
**⚡ เปลี่ยนหน้าช้า ~1.4 วินาที — เจอต้นเหตุและแก้แล้ว (2026-09-16)**
ผู้ใช้ทักว่าเปลี่ยนหน้าแล้วช้า **วัดจริงแล้วพบว่าไม่ปกติ: โค้ดชุดเดียวกันรันบนเครื่องผู้ใช้ 128 ms แต่บน Vercel 1,408 ms (ช้ากว่า 11 เท่า)**

**ต้นเหตุ: Vercel รันฟังก์ชันที่ `iad1` (วอชิงตัน ดี.ซี.) ส่วน Supabase อยู่ `ap-southeast-1` (สิงคโปร์)** → ทุกคำสั่งฐานข้อมูลวิ่งข้ามโลกไปกลับ
(หลงทางตอนแรกเพราะ `x-vercel-id` โชว์ `sin1` — **นั่นคือภูมิภาคของด่านตรวจ (middleware/edge) ไม่ใช่ของฟังก์ชันที่ render หน้า** ต้องดู `process.env.VERCEL_REGION` จากข้างในถึงจะรู้ความจริง)

**วิธีแก้:** เพิ่มไฟล์ `vercel.json` → `{"regions": ["sin1"]}`

| | ก่อนแก้ | หลังแก้ |
|---|---|---|
| เรียกฐานข้อมูล 8 คำสั่งพร้อมกัน | 850-1,010 ms | **59-66 ms** |
| เวลาที่รอตอนกดเปลี่ยนหน้า | 1,148-1,435 ms | **128-153 ms** |

**วิธีวัดที่ใช้ (ทำซ้ำได้):** เซ็น token ทดสอบด้วย `SESSION_SECRET` แล้วยิง request ตรงจาก Node
เทียบ 3 จุด: หน้า static (ไม่แตะฐานข้อมูล) · หน้าที่ถูกเด้งไป login (ผ่านแค่ด่านตรวจ) · หน้าที่ดึงข้อมูลเต็ม → ส่วนต่างบอกได้ว่าเวลาหายไปตรงไหน
เคยสร้าง `/api/perf` ไว้วัดจากข้างในเครื่อง Vercel **ลบทิ้งแล้วหลังหาสาเหตุเจอ** (ถ้าต้องวัดอีกให้สร้างใหม่ชั่วคราวแล้วลบ)

**🚚 กำลังเตรียมย้ายไปบัญชี Vercel/Supabase คนละตัวเลย (เริ่ม 2026-09-16)**
ผู้ใช้ขอย้าย webapp นี้ไปบัญชีใหม่ทั้งสองฝั่ง (อีเมล/บัญชีคนละตัว) — **ตัดสินใจแล้ว: เริ่มฐานข้อมูลใหม่สะอาด ไม่ย้ายข้อมูลทดสอบเก่าไปด้วย**

**เตรียมไว้แล้ว (ไม่กระทบระบบเดิมเลย):**
- `supabase/migrations/0009_seed_first_admin.sql` — ฟังก์ชัน `seed_first_admin(p_password)` แก้ปัญหาไก่กับไข่ของฐานข้อมูลใหม่เอี่ยม (`admin_create_user` ต้องมี admin อยู่ก่อนถึงจะเรียกได้ แต่ฐานข้อมูลใหม่ไม่มีใครเลย)
...
**✅ ฝั่ง Vercel เสร็จแล้วด้วย — ทั้งระบบใหม่ใช้งานได้จริงแล้ว (2026-09-16):**
- Vercel URL ใหม่: **https://workload-tracker-psi.vercel.app**
- ผู้ใช้เพิ่ม GitHub บัญชีที่ Vercel ใหม่ผูกอยู่ (`csoboard321-ai`) เป็น Collaborator บน `beau-bo-bo/workload-tracker` แล้ว import สำเร็จ

**ปัญหาที่เจอจริงระหว่างทาง + วิธีแก้ (บันทึกละเอียดไว้ใน `README.md` หัวข้อย้ายบัญชี เผื่อทำซ้ำ):**
1. **เพิ่ม Collaborator แล้วมองไม่เห็น repo ในหน้าโปรไฟล์ GitHub ของบัญชีที่ถูกเพิ่ม** — เป็นแค่ GitHub UI ไม่โชว์ repo ที่เป็นแค่ collaborator ในแท็บ Repositories ของตัวเอง ไม่ใช่ปัญหาสิทธิ์จริง (ตรวจสอบจริงด้วยการเปิด URL repo ตรง ๆ — เข้าได้)
2. **Vercel Import dropdown มี scope เดียวคือบัญชีตัวเอง ไม่เห็น repo ของ `beau-bo-bo`** — ต้องกด **"+ Add GitHub Scope"** ในดรอปดาวน์ ซึ่งพาไปหน้า `github.com/settings/installations/<id>` ให้เลือก **"Only select repositories"** แล้วเพิ่ม repo เข้าไป **ต้องกด Save จริง ๆ** (เกือบพลาดเพราะเลือก repo ไว้แล้วแต่ยังไม่ได้กด Save)
3. **สำคัญ: repo ของ `beau-bo-bo` ไม่ได้ขึ้นเป็น scope แยกชื่อ `beau-bo-bo`** — เพราะเป็น personal installation ของบัญชีที่ถูกเพิ่ม (`csoboard321-ai`) ไม่ใช่ org ของ `beau-bo-bo` repo จึงโผล่**อยู่ใต้ scope เดิมของบัญชีที่ถูกเพิ่มนั่นเอง** หลัง Save แล้วต้อง **reload หน้า Vercel ทั้งหน้า (F5)** ไม่ใช่แค่ปิดเปิดดรอปดาวน์ (Vercel cache รายการ repo ไว้)
4. **Deploy สำเร็จแต่ login ไม่ผ่านเลย ทั้งรหัสใหม่และรหัสเดิม** — ตรวจ Supabase project logs (`query_logs`, source `postgrest_logs`) แล้วพบว่า **ไม่มี request จากเว็บเข้ามาที่ฐานข้อมูลใหม่เลยแม้แต่ครั้งเดียว** ทั้งที่ตรวจตรงฐานข้อมูลแล้วรหัสผ่านถูกต้อง 100%
   → สาเหตุ: **แก้ Environment Variables หลัง Import ครั้งแรกแล้วไม่ได้กด Redeploy** — Vercel ไม่ใช้ค่า env ใหม่กับ deployment ที่ deploy ไปแล้วโดยอัตโนมัติ ต้องกด Redeploy เองเสมอทุกครั้งที่แก้ env
   **วิธีวินิจฉัยนี้ทำซ้ำได้และแม่นมาก:** เช็ค Supabase project logs ว่ามี traffic จริงไหม ก่อนไล่หาสาเหตุอื่น
- Redeploy แล้ว login ผ่าน ทดสอบครบ: `/board` `/my-tasks` `/dashboard` `/trash` เปิดได้หมด (200 ทุกหน้า)

**⚠️ สถาปัตยกรรมตอนนี้ (สำคัญมาก อ่านก่อนแก้โค้ดครั้งต่อไป):**
ทั้ง 2 ระบบดึงโค้ดจาก **repo/branch `main` เดียวกัน** — **push 1 ครั้ง = ขึ้นทั้ง 2 เว็บพร้อมกันอัตโนมัติเสมอ** ไม่มีทางแก้แค่ระบบเดียว (นอกจากจะแยก branch ซึ่งยังไม่ได้ทำ)
แต่ **ฐานข้อมูลแยกกันเด็ดขาด ไม่ sync** — ข้อมูลที่สร้าง/แก้ในระบบหนึ่งจะไม่โผล่อีกระบบ ดู `README.md` หัวข้อบนสุดสำหรับตารางเทียบละเอียด
ทั้งสองระบบยังใช้งานได้ปกติ **ตามคำสั่งผู้ใช้ให้เก็บระบบเดิมไว้ก่อน** — จะปิด/ลบระบบไหนเมื่อไหร่เป็นคำสั่งแยกต่างหากที่ต้องรอผู้ใช้ยืนยันเอง

**🔑 Session ต่ออายุอัตโนมัติ (ผู้ใช้สั่ง 2026-09-16: "ใช้เบราว์เซอร์เดิม ไม่ต้อง login ใหม่")**
เดิมอายุ 7 วันนับจากวินาทีที่ login → เข้าใช้ทุกวันก็ยังหลุดตอนครบ 7 วัน
ตอนนี้ `middleware.ts` เซ็น token ใบใหม่ให้เมื่อใบเดิมเก่ากว่า 1 วัน (`SESSION_RENEW_AFTER_SECONDS`) นับ 7 วันใหม่จากตอนนั้น
- ค่าคุกกี้ย้ายไปอยู่ที่เดียวกัน `sessionCookieOptions()` ใน `session.ts` (ตอน login กับตอนต่ออายุต้องใช้ชุดเดียวกันเป๊ะ)
- **ทดสอบจริงด้วยการยิง request พร้อม token อายุต่าง ๆ เข้าเซิร์ฟเวอร์:** เพิ่งเซ็น/12 ชม. = ไม่ต่อ · 2 วัน/6.5 วัน = ต่อให้ พร้อม `Max-Age=604800` + `HttpOnly` + `SameSite=lax` ครบ
- test เพิ่ม `src/lib/session.test.ts` 4 ข้อ (รวมเป็น 26 ข้อ)
- ⚠️ **ผลข้างเคียงด้านความปลอดภัย: session ยาวได้ไม่จำกัดตราบใดที่ยังใช้งาน** → งาน **B2 (session ที่ revoke ได้)** สำคัญขึ้นกว่าเดิมมาก ถอดสิทธิ์/ลบผู้ใช้แล้วเขายังใช้ต่อได้จนกว่าจะหยุดใช้เกิน 7 วัน
