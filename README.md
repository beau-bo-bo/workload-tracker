...
## ⚠️ ตอนนี้มี 2 ระบบที่ใช้งานจริงพร้อมกัน (ตั้งแต่ 2026-09-16)

**ใช้โค้ดชุดเดียวกันจาก repo/branch เดียวกัน แต่ฐานข้อมูลแยกกันเด็ดขาด:**

| | ระบบเดิม | ระบบใหม่ |
|---|---|---|
| เว็บ | https://workload-tracker-kappa.vercel.app | https://workload-tracker-psi.vercel.app |
| Vercel account | เดิม (beau-bo-bo) | ใหม่ (csoboard321-2879 / org csoboard) |
| Supabase project | `kalooxclwbnjunuxbjst` | `vohcxhztabjcbdrhwreq` (org csoboard) |
| ข้อมูล | ของเดิมทั้งหมด | ฐานข้อมูลสะอาด เริ่มใหม่ (ไม่มีข้อมูลเก่า) |

```
GitHub (main)
    ├──▶ Vercel บัญชีเดิม  ──▶ workload-tracker-kappa.vercel.app  ──▶ Supabase เดิม
    └──▶ Vercel บัญชีใหม่  ──▶ workload-tracker-psi.vercel.app    ──▶ Supabase ใหม่
```

**กฎที่ต้องรู้:** push ขึ้น `main` ครั้งเดียว = **โค้ดขึ้นทั้ง 2 เว็บพร้อมกันอัตโนมัติ** (Vercel ทั้งสองบัญชีต่างก็ watch branch เดียวกัน)
แต่ **ข้อมูล (ครั้งประชุม/วาระ/ผู้ใช้/memo) ไม่ sync กัน** — สร้างในเว็บหนึ่งจะไม่โผล่อีกเว็บ เพราะเป็นคนละฐานข้อมูล
ทั้งสองระบบยังทำงานอยู่ (ผู้ใช้สั่งให้เก็บระบบเดิมไว้ก่อน 2026-09-16) — จะปิด/ลบระบบไหนเมื่อไหร่เป็นคำสั่งแยกต่างหาก

> push ขึ้น branch `main` เมื่อไหร่ Vercel **ทั้งสองบัญชี** จะ build และ deploy ให้เองอัตโนมัติพร้อมกัน
...
**ทำสำเร็จมาแล้วครั้งหนึ่งเมื่อ 2026-09-16** (ดูผลจริงที่หัวข้อด้านบน) — ข้อมูลด้านล่างปรับตามบทเรียนจริงที่เจอแล้ว ใช้ทำซ้ำได้ถ้าต้องย้ายอีก:
...
- ⚠️ **เช็คก่อนว่าบัญชีใหม่มีโปรเจกต์อื่นอยู่แล้วหรือไม่** — เจอจริงว่าบัญชีที่เพิ่มเข้ามามีโปรเจกต์อื่นของทีมอยู่ก่อนแล้ว (schema/ข้อมูลคนละเรื่องเลย) ต้องสร้างโปรเจกต์ใหม่แยกต่างหาก **ห้ามเผลอรันใส่โปรเจกต์เดิมที่มีอยู่**
...
- ถ้า Vercel บัญชีใหม่ล็อกอินด้วย **GitHub คนละบัญชี**: ไปที่ `github.com/beau-bo-bo/workload-tracker` → Settings → Collaborators → **Add people** → ใส่ username บัญชี GitHub ที่ Vercel ใหม่ผูกอยู่ → ส่งคำเชิญ
  **ฝั่งที่ถูกเชิญต้องกดรับคำเชิญก่อน** (เช็คที่ `github.com/notifications` หรืออีเมล) ไม่งั้นสิทธิ์ยังไม่เปิดจริง
- ถ้าล็อกอินด้วย GitHub บัญชีเดิม (`beau-bo-bo`) แค่ Vercel คนละ account: ข้ามขั้นตอนนี้ได้เลย

**3) Vercel — Import repo (จุดที่เจอปัญหาจริงมากที่สุด)**
- กด **Add New → Project** แล้วเปิดดรอปดาวน์เลือก GitHub scope ที่มุมซ้ายบนของหน้า Import
- **ถ้าเป็น personal installation ของ GitHub บัญชีที่ถูกเพิ่มเป็น collaborator** (ไม่ใช่ org) repo ของเจ้าของเดิม (`beau-bo-bo`) **จะไม่ขึ้นเป็น scope แยก** — มันจะโผล่อยู่**ใต้ scope เดิมของบัญชีที่ถูกเพิ่มนั่นแหละ** หลังจากตั้งค่า Repository access แล้ว (ดูข้อถัดไป) อย่าไปหา scope ชื่อ `beau-bo-bo` เพราะจะไม่เจอ
- ถ้าค้นหา `workload-tracker` แล้วไม่เจอเลย ให้กด **"+ Add GitHub Scope"** หรือ **"Adjust GitHub App Permissions"** → จะเปิดหน้า `github.com/settings/installations/<id>` → เลือก **"Only select repositories"** → เพิ่ม `beau-bo-bo/workload-tracker` เข้าไปในลิสต์ → **ต้องกด Save ด้านล่างจริง ๆ** (พลาดบ่อย: เลือก repo ไว้แล้วแต่ลืมกด Save)
- กลับมาที่ Vercel → **ต้อง reload หน้าทั้งหมด (F5)** ไม่ใช่แค่ปิด-เปิดดรอปดาวน์ Vercel cache รายการ repo ไว้
- ตั้งค่า Environment Variables ให้ใช้ค่าจากโปรเจกต์ Supabase **ใหม่** (ข้อ 1) และ `SESSION_SECRET` ตั้งค่าใหม่ (สุ่มใหม่ ห้ามใช้ค่าเดิมซ้ำข้ามระบบ)

**4) หลัง deploy เสร็จ**
- ⚠️ **ถ้าแก้ Environment Variables ทีหลัง (ไม่ใช่ตอน Import ครั้งแรก) ต้องกด Redeploy เองเสมอ** — Vercel ไม่ใช้ค่าใหม่กับ deployment ที่ deploy ไปแล้วโดยอัตโนมัติ (เจอจริง: ใส่ env ถูกแล้วแต่ login ไม่ผ่าน เพราะยังไม่ได้ redeploy — เช็คได้จาก Supabase project logs ว่ามี request จากเว็บเข้ามาจริงไหม ถ้าไม่มีเลยคือปัญหานี้)
- login ด้วย `admin` + รหัสผ่านที่ตั้งไว้ตอน seed → เปลี่ยนรหัสผ่านทันที
- สร้างผู้ใช้อื่นที่ต้องการผ่านหน้า `/admin/users`
- ระบบเก่ายังใช้งานได้ตามปกติ ไม่ได้ถูกแตะต้องเลยในขั้นตอนนี้ — **แต่โค้ดที่ push ขึ้น `main` หลังจากนี้จะขึ้นทั้ง 2 ระบบพร้อมกันเสมอ** (ดูหัวข้อด้านบนสุดของไฟล์นี้)
